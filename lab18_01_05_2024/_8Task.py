...
def mainFunc(dir, fileName):
    ...
    for i in range(12): # write data to file
        writeLineToFile(str(random.randint(-10,10)), os.path.join(dir, fileName))
    ...
    for el in listLines:
        ...
        if (abs(elInt)>=1) & (abs(elInt)<=5):
            ...
            if len(description) > 0:
                ...
                description = description + f'{el}'
    print(f'From {len(listLines)} numbers {counter} are include [1;5]. And multiplication {description} = {multiplierM}')


currentDir = os.getcwd()
dirSource = os.path.join(currentDir, 'source8')
fileName = 'test8.txt'
